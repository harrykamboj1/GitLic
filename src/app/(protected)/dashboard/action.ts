...
export async function askQuestion(projectId:string,question:string) {
    ...
    if(!queryVector || !projectId || !question){
        return Error("Fail to generate embeddings")
    }
    const vectorQuery = `[${queryVector.join(',')}]`;
    

    // 	1.	summaryEmbedding <=> vectorQuery returns a distance (lower = more similar).
	  //  2.	1 - (summaryEmbedding <=> vectorQuery) converts it into a similarity score (higher = more similar).
    //  3.  The “0.5” is a similarity threshold that will be highly dependent on the use case and requires fine-tuning in real-world applications
